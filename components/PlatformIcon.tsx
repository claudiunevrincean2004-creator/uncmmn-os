'use client';
import { useId } from 'react';

interface Props {
  platform: string;
  size?: number;
}

// Shared rounded "tile": a solid branded background so the glyph always has
// contrast regardless of the card/theme (aurora or midnight) background.
function Tile({ size, bg, children }: { size: number; bg: string; children: React.ReactNode }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, display: 'block' }}>
      <rect x="0" y="0" width="24" height="24" rx="6" fill={bg} />
      {children}
    </svg>
  );
}

function InstagramIcon({ size }: { size: number }) {
  const id = useId();
  const gradId = `ig-grad-${id}`;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, display: 'block' }}>
      <defs>
        <radialGradient id={gradId} cx="30%" cy="107%" r="150%">
          <stop offset="0%" stopColor="#fdf497" />
          <stop offset="5%" stopColor="#fdf497" />
          <stop offset="45%" stopColor="#fd5949" />
          <stop offset="60%" stopColor="#d6249f" />
          <stop offset="90%" stopColor="#285AEB" />
        </radialGradient>
      </defs>
      {/* Brand gradient tile */}
      <rect x="0" y="0" width="24" height="24" rx="6" fill={`url(#${gradId})`} />
      {/* White camera glyph */}
      <rect x="6" y="6" width="12" height="12" rx="3.6" stroke="#fff" strokeWidth="1.7" fill="none" />
      <circle cx="12" cy="12" r="3.1" stroke="#fff" strokeWidth="1.7" fill="none" />
      <circle cx="15.8" cy="8.2" r="1.05" fill="#fff" />
    </svg>
  );
}

function TikTokIcon({ size }: { size: number }) {
  return (
    <Tile size={size} bg="#000000">
      {/* White music-note glyph, inset onto the black tile */}
      <g transform="translate(3.6 3.6) scale(0.7)" fill="#fff">
        <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
      </g>
    </Tile>
  );
}

function YouTubeIcon({ size }: { size: number }) {
  return (
    <Tile size={size} bg="#FF0000">
      {/* White play triangle */}
      <path d="M10 8.4v7.2l6.2-3.6z" fill="#fff" />
    </Tile>
  );
}

export default function PlatformIcon({ platform, size = 16 }: Props) {
  const p = platform.toLowerCase();
  if (p === 'instagram') return <InstagramIcon size={size} />;
  if (p === 'tiktok') return <TikTokIcon size={size} />;
  if (p === 'youtube') return <YouTubeIcon size={size} />;
  // Neutral tile fallback for unknown platforms — still legible in both themes.
  return (
    <Tile size={size} bg="#6B7280">
      <text x="12" y="16.5" textAnchor="middle" fontSize="12" fontWeight="700" fill="#fff" fontFamily="var(--font-ui)">
        {platform.charAt(0).toUpperCase()}
      </text>
    </Tile>
  );
}
